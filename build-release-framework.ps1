# OpenKTV - Framework-Dependent Release Build Script
# This script builds a release package that requires .NET 8 to be installed

param(
    [string]$OutputDir = "release/OpenKTV-Framework",
    [switch]$SkipClean
)
...
# Step 2: Build the application (framework-dependent)
Write-Host "Step 2: Building application (Release/x64 - Framework Dependent)..." -ForegroundColor Yellow
$buildArgs = @(
    "publish",
    "src/UI/Karaoke.UI/Karaoke.UI.csproj",
    "-c", "Release",
    "-r", "win-x64",
...
$readmeContent = @"
OpenKTV - Release Version (Framework-Dependent)
============================================================

This version requires .NET 8 Runtime to be installed on your system.

System Requirements:
- Windows 10 or later (64-bit)
- .NET 8 Runtime (Desktop) - Download from: https://dotnet.microsoft.com/download/dotnet/8.0

Installation:
1. Install .NET 8 Runtime (Desktop) if not already installed
   Download: https://dotnet.microsoft.com/download/dotnet/8.0/runtime
   Choose: ".NET Desktop Runtime 8.0.x - Windows x64"

2. Copy this entire folder to any location on your PC

3. Double-click "OpenKTV.exe" to start the application

4. On first run, the application will create:
...
Advantages of Framework-Dependent Version:
- Much smaller download size (~100 MB vs ~435 MB)
- Automatic updates through Windows Update
- Better performance with ReadyToRun disabled
...
- You can safely move this folder to another PC (that has .NET 8 installed)
- Drive Override changes require app restart to take effect

Troubleshooting:
- If the app doesn't start, make sure .NET 8 Desktop Runtime is installed
- Check: https://dotnet.microsoft.com/download/dotnet/8.0
...
Write-Host "Total Size: $sizeMB MB (Framework-Dependent)" -ForegroundColor White
Write-Host ""
Write-Host "IMPORTANT:" -ForegroundColor Yellow
Write-Host "This build requires .NET 8 Desktop Runtime to be installed!" -ForegroundColor White
Write-Host "Download: https://dotnet.microsoft.com/download/dotnet/8.0" -ForegroundColor White
Write-Host ""
Write-Host "Next Steps:" -ForegroundColor Yellow
Write-Host "1. Test the application by running: $OutputDir/OpenKTV.exe" -ForegroundColor White
Write-Host "   (Make sure .NET 8 Runtime is installed on test machine)" -ForegroundColor White
Write-Host "2. Zip the '$OutputDir' folder for distribution" -ForegroundColor White
Write-Host "3. Include .NET 8 installation instructions for users" -ForegroundColor White
Write-Host ""
